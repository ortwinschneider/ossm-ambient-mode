...
NOTE: To get a better understanding of the application architecture and components used throughout the lab, please refer to the following page: xref:module-4-about.adoc[Understanding the Travel Demo Application]
...
image:kiali-initial-traffic-graph.png[kiali-initial-traffic-graph.png]
...
It enables probabilistic sampling, capturing 50% of requests that pass through that Gateway or waypoint.
Other workloads or gateways are not affected unless they have their own Telemetry config.
...
image:jaeger-traces.png[jaeger-traces.png]

NOTE: Congratulations! You have successfully deployed the Kiali Travel Demo application in Ambient Mesh with an Ingress Gateway and Waypoint Proxy!